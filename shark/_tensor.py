...
class Tensor(metaclass=TorchTensorWrapper):
    ...
    def __add__(self, *args, **kwargs):
        return shark.add_Tensor(self, args[0])
    ...
    def __mul__(self, *args, **kwargs):
        return shark.mul_Tensor(self, args[0])
...
def empty(shape: Tuple[int, ...], dtype: "shark.dtype" = None, **kwargs) -> Tensor:
    if np.prod(shape) == 0:
        return Tensor(None)
    else:
...
def ones(*size: Tuple[int, ...], **kwargs) -> Tensor:
    # dtype: "shark.dtype" = None, _device: Any = None
    dtype = kwargs.get("dtype", None)
    if dtype is not None:
        dtype = dtype.to_np_type()
...
def tensor(data: Any, dtype: Optional["shark.dtype"] = None) -> Tensor:
    if dtype is not None:
        dtype = dtype.to_np_type()
