...
from libcst.metadata.type_inference_provider import Position
from libcst.testing.utils import data_provider, UnitTest

from shark.compiler.providers.scope import MyScopeProvider


class DependentVisitor(cst.CSTVisitor):
    METADATA_DEPENDENCIES = (MyScopeProvider,)


def get_scope_metadata_provider(
    module_str: str,
) -> tuple[cst.Module, Mapping[cst.CSTNode, Scope], Mapping[cst.CSTNode, Scope]]:
    wrapper = MetadataWrapper(cst.parse_module(dedent(module_str)))
    return (
        wrapper.module,
        cast(
            Mapping[cst.CSTNode, Scope], wrapper.resolve(MyScopeProvider)
        ),  # we're sure every node has an associated scope
        cast(
            Mapping[cst.CSTNode, Position], wrapper.resolve(PositionProvider)
        ),  # we're sure every node has an associated scope
    )


class ForScopeTest(UnitTest):
    def test_for_scope_ordering(self) -> None:
        m, scopes, _positions = get_scope_metadata_provider(
            """
            def f():
                for x in []:
                    for y in []:
        ...
        for scope in scopes.values():
            ...
            for acc in scope.accesses:
                self.assertEqual(
                    # Return all assignments of the access (defs?)
                    len(acc.referents),
                    1,
                    msg=(
...
class FullCSTScopeProviderTest(UnitTest):
