    ...
    def __init__(
        ...
        if self.track_running_stats:
            self.register_buffer(
                "running_mean",
                UninitializedBuffer(shark.zeros, (num_features,), **factory_kwargs),
            )
            self.register_buffer(
                "running_var",
                UninitializedBuffer(shark.ones, (num_features,), **factory_kwargs),
            )
    ...
    def _load_from_state_dict(
        ...
        if (version is None or version < 2) and self.track_running_stats:
            ...
            if num_batches_tracked_key not in state_dict:
                state_dict[num_batches_tracked_key] = shark.tensor(0, dtype=shark.long)
...
#             self.num_batches_tracked = shark.tensor(
#                 0,
#                 dtype=shark.long,
#                 **{k: v for k, v in factory_kwargs.items() if k != "dtype"},
#             )
...
class SyncBatchNorm(_BatchNorm):
    ...
    def forward(self, input: Tensor) -> Tensor:
        ...
        need_sync = (
            bn_training
            and self.training
            and shark.distributed.is_available()
            and shark.distributed.is_initialized()
        )
        if need_sync:
            ...
            process_group = shark.distributed.group.WORLD
            if self.process_group:
                process_group = self.process_group
            world_size = shark.distributed.get_world_size(process_group)
    ...
    def convert_sync_batchnorm(cls, module, process_group=None):
        ...
        if isinstance(module, shark.nn.modules.batchnorm._BatchNorm):
            module_output = shark.nn.SyncBatchNorm(
                module.num_features,
                module.eps,
                module.momentum,
                module.affine,
