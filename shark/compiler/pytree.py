...
#   It can also return some Any that is used in reconstructing the
#   collection.
# - unflatten_fn should take a flat list of values and some Any
#   (returned by flatten_fn). It returns the collection by reconstructing
#   it from the list and the Any.
PyTree = Any
...
# Any: some Any that is useful in unflattening the pytree
...
def tree_flatten(pytree: PyTree) -> Tuple[List[Any], TreeSpec]:
    ...
    return result, TreeSpec(node_type, Any, children_specs)
...
def _broadcast_to_and_flatten(pytree: PyTree, spec: TreeSpec) -> Optional[List[Any]]:
    ...
    if len(child_pytrees) != len(spec.children_specs) or ctx != spec.Any:
        return None
