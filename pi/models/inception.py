...
class Inception3(nn.Module):
    def __init__(
        ...
        #             pi.nn.init.trunc_normal_(m.weight, mean=0.0, std=stddev, a=-2, b=2)
    ...
    def _transform_input(self, x: Tensor) -> Tensor:
        if self.transform_input:
            x_ch0 = pi.unsqueeze(x[:, 0], 1) * (0.229 / 0.5) + (0.485 - 0.5) / 0.5
            x_ch1 = pi.unsqueeze(x[:, 1], 1) * (0.224 / 0.5) + (0.456 - 0.5) / 0.5
            x_ch2 = pi.unsqueeze(x[:, 2], 1) * (0.225 / 0.5) + (0.406 - 0.5) / 0.5
            x = pi.cat((x_ch0, x_ch1, x_ch2), 1)
        return x
...
class InceptionE(nn.Module):
    ...
    def _forward(self, x: Tensor) -> List[Tensor]:
        ...
        branch3x3dbl = pi.cat(branch3x3dbl, 1)
