...
def clamp(
    self_: Tensor, min: Optional[Number] = None, max: Optional[Number] = None
) -> Tensor:
...
def addcmul(
    self_: Tensor, tensor1: Tensor, tensor2: Tensor, value: Number = 1
) -> Tensor:
...
def index_put_hacked_twin(
    self_: Tensor, indices: List[Tensor], values: Tensor, accumulate: bool = False
) -> Tensor:
    return Tensor(
        torch_dialect.AtenIndexPutHackedTwinOp(self_, indices, values, accumulate)
    )


def index_put__hacked_twin(
    self_: Tensor, indices: List[Tensor], values: Tensor, accumulate: bool = False
) -> Tensor:
    return Tensor(
        torch_dialect.AtenIndexPut_HackedTwinOp(self_, indices, values, accumulate)
    )
...
def addmm(
    self_: Tensor, mat1: Tensor, mat2: Tensor, beta: Number = 1, alpha: Number = 1
) -> Tensor:
...
def native_layer_norm(
    ...
    return Tensor(
        torch_dialect.AtenNativeLayerNormOp(input, normalized_shape, weight, bias, eps)
    )
...
def topk(
    self_: Tensor, k: int, dim: int = -1, largest: bool = True, sorted: bool = True
) -> Tuple[Tensor, Tensor]:
    return Tensor(torch_dialect.AtenTopkOp(self_, k, dim, largest, sorted))
...
def var_mean(self_: Tensor, unbiased: bool = True) -> Tuple[Tensor, Tensor]:
    return Tensor(torch_dialect.AtenVarMeanOp(self_, unbiased))
...
def nll_loss_forward(
    ...
    return Tensor(
        torch_dialect.AtenNllLossForwardOp(
            self_, target, weight, reduction, ignore_index
        )
    )
...
def nll_loss_backward(
    ...
    return Tensor(
        torch_dialect.AtenNllLossBackwardOp(
            grad_output, self_, target, weight, reduction, ignore_index, total_weight
        )
    )


def bincount(
    self_: Tensor, weights: Optional[Tensor] = None, minlength: int = 0
) -> Tensor:
...
def pad(
    self_: Tensor, pad: List[int], mode: str = "constant", value: Optional[float] = None
) -> Tensor:
...
def arange_start_out(
    start: Number, end: Number, step: Number = 1, out: Tensor = None
) -> Tensor:
...
def bucketize_Tensor(
    self_: Tensor, boundaries: Tensor, out_int32: bool = False, right: bool = False
) -> Tensor:
    return Tensor(
        torch_dialect.AtenBucketizeTensorOp(self_, boundaries, out_int32, right)
    )
...
def resize_(
    self_: Tensor, size: List[int], memory_format: Optional[int] = None
) -> Tensor:
...
def max_dim(self_: Tensor, dim: int, keepdim: bool = False) -> Tuple[Tensor, Tensor]:
    return Tensor(torch_dialect.AtenMaxDimOp(self_, dim, keepdim))
...
def to_dtype(
    ...
    return Tensor(
        torch_dialect.AtenToDtypeOp(self_, dtype, non_blocking, copy, memory_format)
    )
...
def to_other(
    ...
    return Tensor(
        torch_dialect.AtenToOtherOp(self_, other, non_blocking, copy, memory_format)
    )
...
def native_dropout(
    input: Tensor, p: float, train: Optional[bool]
) -> Tuple[Tensor, Tensor]:
    return Tensor(torch_dialect.AtenNativeDropoutOp(input, p, train))
...
def baddbmm(
    self_: Tensor, batch1: Tensor, batch2: Tensor, beta: Number = 1, alpha: Number = 1
) -> Tensor:
...
def fft_fft(
    self_: Tensor, n: Optional[int] = None, dim: int = -1, norm: Optional[str] = None
) -> Tensor:
...
def as_strided_copy(
    ...
    return Tensor(
        torch_dialect.AtenAsStridedCopyOp(self_, size, stride, storage_offset)
    )


def diagonal_copy(
    self_: Tensor, offset: int = 0, dim1: int = 0, dim2: int = 1
) -> Tensor:
...
def diagonal_scatter(
    self_: Tensor, src: Tensor, offset: int = 0, dim1: int = 0, dim2: int = 1
) -> Tensor:
...
def as_strided_scatter(
    ...
    return Tensor(
        torch_dialect.AtenAsStridedScatterOp(self_, src, size, stride, storage_offset)
    )
...
def upsample_nearest2d(
    ...
    return Tensor(
        torch_dialect.AtenUpsampleNearest2dOp(self_, output_size, scales_h, scales_w)
    )
...
def insert_t(self_: List[Tensor], idx: int, el: Tensor) -> None:
    Tensor(torch_dialect.AtenInsertTOp(self_, idx, el))
...
def sort_int(self_: List[int], reverse: bool = False) -> None:
    Tensor(torch_dialect.AtenSortIntOp(self_, reverse))
...
def _softmax_backward_data(
    ...
    return Tensor(
        torch_dialect.Aten_SoftmaxBackwardDataOp(grad_output, output, dim, input_dtype)
    )
...
def gelu_backward(
    grad_output: Tensor, self_: Tensor, approximate: str = "none"
) -> Tensor:
...
def prim_RaiseException(msg: str, cls: Optional[str] = None) -> None:
    Tensor(torch_dialect.PrimRaiseExceptionOp(msg, cls))
